...
const TypingGame: React.FC = () => {
  ...
  useEffect(() => {
    const handleSpeedControl = (e: KeyboardEvent) => {
    ...
    window.addEventListener('keydown', handleSpeedControl);
    return () => window.removeEventListener('keydown', handleSpeedControl);
  }, [gameState, playTypeSound]);
      ...
      <div className="w-full max-w-2xl p-8 bg-gradient-to-b from-blue-100 to-blue-200 shadow-xl rounded-lg transition-transform duration-300 hover:scale-[1.02]">
        <div className="text-center mb-4">
          ...
          <div className="flex justify-between items-center px-4">
            <div>
              <p className="text-lg">ステージ {stage}/10</p>
          ...
          {gameState === 'start' && (
            <div className="absolute inset-0 bg-gradient-to-b from-blue-500 to-blue-700 flex flex-col items-center justify-center text-white">
              ...
              <p className="text-lg mb-6">
                スピード: {speedMultiplier} 　(1〜5キーで変更可能)
              </p>
              <button
                onClick={resetGame}
                className="bg-green-500 hover:bg-green-600 text-white text-xl py-6 px-8 rounded-lg transform transition-all duration-300 hover:scale-110 hover:rotate-1"
              >
                スタート！
              </button>
              <p className="text-gray-200 mt-4 animate-pulse">
                スペースキーでもスタートできます
              </p>
            </div>
          )}
